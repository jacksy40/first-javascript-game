...
def initialize
  ...
  @select = nil
  @functions = [
                "user_des","user_length_last_tweet",
                "user_length_last_twenty_tweets","most_followers",
                "most_friends","greatest_number_of_tweets","people_with_des",
                "with_location"
  ]
...
def users
  ...
  puts "#{@users.count} UserID"
  @users.each do |array|
    if @user_count % 3==0
      puts
      print"#{ @user_count += 1 }.) #{array}".ljust(25)
    else
      print"#{@user_count += 1}.) #{array}".ljust(25)
    end
  end
...
def function
  puts "Collective Queries - format (#)"
  print "1.) Description".ljust(25)
  print "2.) Length of last tweet".ljust(25)
  print "3.) Length of last 20 tweets\n".ljust(25)
  print "4.) Most followers".ljust(25)
  print "5.) Most friends".ljust(25)
  print "6.) Most tweets".ljust(25)
  puts
  print "7.) With description".ljust(25)
  print "8.) Location".ljust(25)
  puts
  puts "_" * 80
end
...
def query
  ...
  q_function = nil
  until q_function =~ /[1,2,3,4,5,6,7,8]/
    puts "please select the query by the corresponding number"
    q_function = gets.chomp
  end
  send @funcions["#{q_function.to_i - 1}"]

end

def user_des
  twitter_data.each do |data|
    data.each do |user,description|
      des=data[user]["description"]
      if des==nil
        puts"#{user}: *description n/a*"
        puts
      else
        puts"#{user}:\n #{des}"
        puts
...
def user_followers
  twitter_data.each do |data|
    data.each do |user,followers|
      followers=data[user]["number of followers"]
      if followers==nil
        puts"#{user}: Poor guy, someone follow him!"
        puts
      else
        puts"#{user}: has #{followers} followers."
        puts
      end
    end
  end
...
def user_length_last_tweet
  twitter_data.each do |data|
    data.each do |user,last|
      latest=data[user]["latest tweet"]
      if latest==nil
        puts"#{user}: *tweet n/a*"
        puts
      else
        puts"#{user}: The last tweet was #{latest.length} characters long."
        puts
      end
    end
  end
...
def user_length_last_twenty_tweets
  twitter_data.each do |data|
    data.each do |user,latest|
      last=data[user]["last twenty tweets"].reduce(:+).length
      if last==nil
        puts"#{user}: *tweet n/a*"
        puts
      else
        puts"#{user}: The last 20 tweet was #{last} characters long."
        puts
      end
    end
  end
...
hash={}
  twitter_data.each do |data|
    data.each do |users,v|
      @followers=data[users]["number of followers"]
      hash[users]=@followers
    end
  end
puts hash.max_by{|user,num|}
continue
end
...
hash={}
  twitter_data.each do |data|
    data.each do |users,v|
      friends=data[users]["number of friends"]
      hash[users]=friends
    end
  end
puts hash.max_by{|user,num|}
...
def greatest_number_of_tweets
  hash={}
    twitter_data.each do |data|
      data.each do |users,v|
        tweets=data[users]["number of tweets"]
        hash[users]=tweets
      end
    end
puts hash.max_by{|user,num|}
...
def people_with_des
  twitter_data.each{|data|data.each{|user,des| puts user if des["description"]}}
  continue
end

def with_location
  twitter_data.each{|data|data.each{|user,locate| puts user if locate["location"]}}
  continue
end
...
def continue
  cont=nil
  until cont=="q" || cont==""
    puts "please use (q) to quit or enter to continue"
    cont=gets.chomp
  end
    cont=="q"? return: Query.new
  end
end
Query.new
